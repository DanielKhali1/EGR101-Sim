 ...
 *
 *
 *         plan right now would be to use this class to house the Pins and
 *         actions related to pins,power,ground,dig,analog Arduino Behavior
 *         Manager should be composed of this class such that we can insert
 *         behavior into the Arduino may need to add
 */
public class BaseArduino {
	 ...
	 * approximately 70 minutes. On the boards from the Arduino Portenta family this
	 * function has a resolution of one microsecond on all cores. On 16 MHz Arduino
	 * boards (e.g. Duemilanove and Nano), this function has a resolution of four
	 * microseconds (i.e. the value returned is always a multiple of four). On 8 MHz
	 * Arduino boards (e.g. the LilyPad), this function has a resolution of eight
	 * microseconds.
	 * 
	 * @return
	 */
	 ...
	 * after approximately 50 days. (25 in this program since we're using a signed
	 * long
	 * 
	 * @return
	 */
	 ...
	 * input voltages between 0 and the operating voltage(5V or 3.3V) into integer
	 * values between 0 and 1023. On an Arduino UNO, for example, this yields a
	 * resolution between readings of: 5 volts / 1024 units or, 0.0049 volts (4.9
	 * mV) per unit. See the table below for the usable pins, operating voltage and
	 ...
	 * 
	 * pin: the Arduino pin to write to. Allowed data types: int. value: the duty
	 * cycle: between 0 (always off) and 255 (always on). Allowed data types: int.
	 * 
	 * @param analogPin
	 * @param val
	 */
	 ...
	 * the Digital Pins page for details on the functionality of the pins. As of
	 * Arduino 1.0.1, it is possible to enable the internal pullup resistors with
...
//		System.out.println(getAnalogArray()[0]);
	 ...
	 * be set to the corresponding value: 5V (or 3.3V on 3.3V boards) for HIGH,
	 * 
	 * 0V (ground) for LOW.
	 * 
	 * @param pin
